    ...
    def __init__(self,config_path) :
        ...
        def correct_word_s(input_words):  
            ...
            if len(record_possible_word) != 0:
                ...
                elif len(word__wrong_list) > 1:
                    for i in range(len(word__wrong_list)):
                        for item in record_possible_word[i]:
                            if i == 0:
                                new_word = item + word__wrong_list[1]           
                            else:
                                new_word = word__wrong_list[0] + item
                            all_edit_words.append(new_word)
                else:
                    for item in record_possible_word:            
                        all_edit_words.append(item)        
            else:
                pass
            ...
            for text in sentence_list:
                ...
                for i in range(len(edit_word_list)):
                    ...
                    if wrong_word_combine[i] != edit_word_list[i]:    
                        ...
                        edit_word["old_word"] = wrong_word_combine[i].replace('<s/>','')
                        edit_word["new_word"] = edit_word_list[i].replace('<s/>','')
                        result.append(edit_word)

                ans_list.append(result)
            ans_list = [remove_overlap(sentence_list[index],ans_list[index])[:3] for index in  range(len(ans_list))]
